...
#include "fdlist.h"
#include "throttle.h"
#include <sys/socket.h> /* AZZURRA */

/* IRCDCONF_DELIMITER -> AZZURRA */
...
int m_server(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	 ...
	 * cleaning up ones from users, because we will never see them
	 * any more - Dianora
	 */
	...
	*d = '\0';
	 ...
	 *
	 * Deprecated.  Kinda redundant with Hlines. -epi
	 */
...
int do_server_estab(aClient *cptr)
{
     ...
     * to this other fragment at the same time, it's a race condition,
...
int m_server_estab(aClient *cptr)
{
	...
	*s = '@';
    ...
    throttle_remove(inet_ntop(AFINET, (char *)&cptr->ip, mydummy,
	    sizeof(mydummy)));
...
int m_info(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	if (IsAnOper(sptr))
	{
	    ...
	    sendto_one(sptr, ":%s %d %s :ANTI_NICK_FLOOD=%d ANTI_SPAMBOT=%d CLIENT_COUNT=%d",
		    me.name, RPL_INFO, parv[0], ANTI_NICK_FLOOD_VAL, ANTI_SPAMBOT_VAL, CLIENT_COUNT_VAL);

	    sendto_one(sptr, ":%s %d %s :CLIENT_FLOOD=%d CLIENT_SERVER=%d CUSTOM_ERR=%d",
		    me.name, RPL_INFO, parv[0], CLIENT_FLOOD_VAL, CLIENT_SERVER_VAL, CUSTOM_ERR_VAL);
	    
	    sendto_one(sptr, ":%s %d %s :ZLINES_IN_KPATH=%d DNS_DEBUG=%d DO_IDENTD=%d",
		    me.name, RPL_INFO, parv[0], ZLINES_IN_KPATH_VAL, DNS_DEBUG_VAL, DO_IDENTD_VAL);

	    sendto_one(sptr, ":%s %d %s :E_LINES_OPER_ONLY=%d FAILED_OPER_NOTICE=%d FLUD=%d",
		    me.name, RPL_INFO, parv[0], E_LINES_OPER_ONLY_VAL, FAILED_OPER_NOTICE_VAL, FLUD_VAL);
	    
	    sendto_one(sptr, ":%s %d %s :SHORT_MOTD=%d F_LINES_OPER_ONLY=%d HIGHEST_CONNECTION=%d",
		    me.name, RPL_INFO, parv[0], SHORT_MOTD_VAL, F_LINES_OPER_ONLY_VAL, HIGHEST_CONNECTION_VAL);

	    sendto_one(sptr, ":%s %d %s :HUB=%d IDENTD_COMPLAIN=%d IGNORE_FIRST_CHAR=%d",
		    me.name, RPL_INFO, parv[0], HUB_VAL, IDENTD_COMPLAIN_VAL, IGNORE_FIRST_CHAR_VAL);

	    sendto_one(sptr, ":%s %d %s :KPATH=%d LOCKFILE=%d MAXBUFFERS=%d",
		    me.name, RPL_INFO, parv[0], KPATH_VAL, LOCKFILE_VAL, MAXBUFFERS_VAL);

	    sendto_one(sptr, ":%s %d %s :NON_REDUNDANT_KLINES=%d NO_CHANOPS_WHEN_SPLIT=%d NO_DEFAULT_INVISIBLE=%d",
		    me.name, RPL_INFO, parv[0], NON_REDUNDANT_KLINES_VAL, NO_CHANOPS_WHEN_SPLIT_VAL, NO_DEFAULT_INVISIBLE_VAL);

	    sendto_one(sptr, ":%s %d %s :NO_MIXED_CASE=%d NO_OPER_FLOOD=%d NO_PRIORITY=%d",
		    me.name, RPL_INFO, parv[0], NO_MIXED_CASE_VAL, NO_OPER_FLOOD_VAL, NO_PRIORITY_VAL);

	    sendto_one(sptr, ":%s %d %s :OLD_Y_LIMIT=%d REJECT_IPHONE=%d RFC1035_ANAL=%d",
		    me.name, RPL_INFO, parv[0], OLD_Y_LIMIT_VAL, REJECT_IPHONE_VAL, RFC1035_ANAL_VAL);

	    sendto_one(sptr, ":%s %d %s :RIDICULOUS_PARANOIA_LEVEL=%d, SEPARATE_QUOTE_KLINES_BY_DATE=%d",
		    me.name, RPL_INFO, parv[0], RIDICULOUS_PARANOIA_LEVEL, SEPARATE_QUOTE_KLINES_BY_DATE_VAL);
...
#ifdef FLUD
	    sendto_one(sptr, ":%s %d %s :FLUD_BLOCK=%d FLUD_NUM=%d FLUD_TIME=%d",
		    me.name, RPL_INFO, parv[0], FLUD_BLOCK, FLUD_NUM, FLUD_TIME);
#endif

	    sendto_one(sptr, ":%s %d %s :HARD_FDLIMIT_=%d HYBRID_SOMAXCONN=%d",
		    me.name, RPL_INFO, parv[0], HARD_FDLIMIT_, HYBRID_SOMAXCONN);

	    sendto_one(sptr, ":%s %d %s :MOTD_WAIT=%d MOTD_MAX=%d MAXIMUM_LINKS=%d",
		    me.name, RPL_INFO, parv[0], MOTD_WAIT, MOTD_MAX, MAXIMUM_LINKS);

	    sendto_one(sptr, ":%s %d %s :MAX_NICK_CHANGES=%d MAX_NICK_TIME=%d",
		    me.name, RPL_INFO, parv[0], MAX_NICK_CHANGES, MAX_NICK_TIME);

	    sendto_one(sptr, ":%s %d %s :NICKNAMEHISTORYLENGTH=%d NOISY_HTM=%d",
		    me.name, RPL_INFO, parv[0], NICKNAMEHISTORYLENGTH, NOISY_HTM);
	    
	    sendto_one(sptr, ":%s %d %s :MAXSENDQLENGTH=%d BUFFERPOOL=%d INIT_MAXCLIENTS=%d",
		    me.name, RPL_INFO, parv[0], MAXSENDQLENGTH, BUFFERPOOL, INIT_MAXCLIENTS);

	    sendto_one(sptr, ":%s %d %s :TS_MAX_DELTA=%d TS_WARN_DELTA=%d",
		    me.name, RPL_INFO, parv[0], TS_MAX_DELTA, TS_WARN_DELTA);
...
int m_links(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	*d = '\0';
    ...
    if (MyConnect(sptr))
	sendto_realops_lev(SPY_LEV, "LINKS %s requested by %s (%s@%s) [%s]",
		mask ? clean_mask : "all", sptr->name, sptr->user->username,
...
static char *get_oflags_str(int flags)
{
    ...
    /* i = 6 because the first three entries of oper_access are ignored
     */
    for(i = &oper_access[0] + 6; *i; i += 2)
    {
	if(flags & *i)
	{
	    *s++ = (char) *(i + 1);
...
int m_stats(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	{
	    Spam *s;
	    char *typetext[] = {"Block, no action",
		                "Block, snotice",
		                "Block, security",
		                "Block, snotice and security",
		                "Allow, security",
		                "Block, snotice, security, auto-shun"};
	   
		
	    for(s = spam_list; s != NULL; s = s->next)
	    {
		sendto_one(sptr, rpl_str(RPL_STATSSPAM), me.name, parv[0],
			s->daycount, s->weekcount, s->monthcount, s->count, 
			s->type, s->msg, s->reason, 
			((s->type >=0) && (s->type <=5)) ? 
			   typetext[s->type] : "Not valid");
	    }
	}
	break;
    case 's':
...
int m_snotice(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
    ...
    sendto_serv_butone_services(IsServer(cptr) ? cptr : NULL, ":%s SNOTICE :%s",
		       parv[0], message);
...
#ifdef ANTI_INVITE_FLOOD  /* defined in channel.c */
extern int  invite_num;
extern int  invite_time;
extern int  invite_spam;
#endif
...
int m_set(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	else if (!strncasecmp(command, "SPLITDELAY", 10)) 
	{
		...
		sendto_ops("%s has changed SERVER SPLIT RECOVERY TIME "
			   "to %i", parv[0], newval);
	...
	else if (!strncasecmp(command, "SHOW_RESTRICTED_JOINS", 21)) 
	{
		...
		{
		    show_restricted_joins = YES;
		}
		else
		{
		    show_restricted_joins = NO;
		}
		sendto_realops_lev(SPAM_LEV, "%s has changed show restricted joins to %s",
			parv[0], show_restricted_joins ? "ON" : "OFF");
		sendto_one(sptr, ":%s NOTICE %s :show restricted joins is now set to %s",
			   me.name, parv[0], show_restricted_joins ? "ON" : "OFF");
		return 0;
	    }
	    else 
	    {
		sendto_one(sptr, ":%s NOTICE %s :show restricted joins is currently %s",
			   me.name, parv[0], show_restricted_joins ? "enabled" : "disabled");
		return 0;
	    }
	}
	else if (!strncasecmp(command, "RESTRICTED_JOINS_NUM", 20)) 
	{
	    if (parc > 2) 
	    {
		int newval = atoi(parv[2]);
		if (newval <= 0) 
		{
		    sendto_one(sptr, ":%s NOTICE %s :resricted joins NUM must be > 0",
			       me.name, parv[0]);
		    return 0;
		}
		restricted_joins_num = newval;
		sendto_ops("%s has changed restricted joins NUM to %i", parv[0], restricted_joins_num);
		sendto_one(sptr, ":%s NOTICE %s :restricted joins NUM is now set to %i",
			   me.name, parv[0], restricted_joins_num);
		return 0;
	    }
	    else
	    {
		sendto_one(sptr, ":%s NOTICE %s :restricted joins NUM is currently %i",
			   me.name, parv[0], restricted_joins_num);
		return 0;
	    }
	}
	else if (!strncasecmp(command, "RESTRICTED_JOINS_TIME", 21)) 
	{
	    if (parc > 2) 
	    {
		int newval = atoi(parv[2]);
		if (newval <= 0) 
		{
		    sendto_one(sptr, ":%s NOTICE %s :restricted joins TIME must be > 0",
			       me.name, parv[0]);
		    return 0;
		}
		restricted_joins_time = newval;
		sendto_ops("%s has changed restricted joins TIME to %i", parv[0],
			   restricted_joins_time);
		sendto_one(sptr, ":%s NOTICE %s :restricted joins TIME is now set to %i",
			   me.name, parv[0], restricted_joins_time);
		return 0;
	    }
	    else 
	    {
		sendto_one(sptr, ":%s NOTICE %s :restricted joins TIME is currently %i",
			   me.name, parv[0], restricted_joins_time);
		return 0;
	    }
	}	
	...
	else if (!strncasecmp(command, "INVITESPAM", 10)) 
	{
		...
		sendto_realops_lev(SPAM_LEV, "%s has changed invite spam to %s",
			parv[0], invite_spam ? "ON" : "OFF");
		sendto_one(sptr, ":%s NOTICE %s :invite spam is now set to %s",
			   me.name, parv[0], invite_spam ? "ON" : "OFF");
		return 0;
	    }
	    else 
	    {
		sendto_one(sptr, ":%s NOTICE %s :invite spam is currently %s",
			   me.name, parv[0], invite_spam ? "enabled" : "disabled");
		return 0;
	    }
	}
	else if (!strncasecmp(command, "INVITENUM", 9)) 
	{
	    if (parc > 2) 
	    {
		int newval = atoi(parv[2]);
		if (newval <= 0) 
		{
		    sendto_one(sptr, ":%s NOTICE %s :invite NUM must be > 0",
			       me.name, parv[0]);
		    return 0;
		}
		invite_num = newval;
		sendto_ops("%s has changed invite NUM to %i", parv[0], invite_num);
		sendto_one(sptr, ":%s NOTICE %s :invite NUM is now set to %i",
			   me.name, parv[0], invite_num);
		return 0;
	    }
	    else
	    {
		sendto_one(sptr, ":%s NOTICE %s :invite NUM is currently %i",
			   me.name, parv[0], invite_num);
		return 0;
	    }
	}
	else if (!strncasecmp(command, "INVITETIME", 10)) 
	{
	    if (parc > 2) 
	    {
		int newval = atoi(parv[2]);
		if (newval <= 0) 
		{
		    sendto_one(sptr, ":%s NOTICE %s :invite TIME must be > 0",
			       me.name, parv[0]);
		    return 0;
		}
		invite_time = newval;
		sendto_ops("%s has changed invite TIME to %i", parv[0],
			   invite_time);
		sendto_one(sptr, ":%s NOTICE %s :invite TIME is now set to %i",
			   me.name, parv[0], invite_time);
		return 0;
	    }
	    else 
	    {
		sendto_one(sptr, ":%s NOTICE %s :invite TIME is currently %i",
			   me.name, parv[0], invite_time);
		return 0;
	    }
	}	
	...
	else if (!strncasecmp(command, "SPAMDETECT", 10)) 
	{
		...
		sendto_realops_lev(SPAM_LEV, "%s has changed spam detection to %s",
			parv[0], spam_detect ? "ON" : "OFF");
		sendto_one(sptr, ":%s NOTICE %s :spam detection is now set to %s",
			   me.name, parv[0], spam_detect ? "ON" : "OFF");
		return 0;
	    }
	    else 
	    {
	...
	else if (!strncasecmp(command, "UNKNOWN_LIST", 12))
	{
	    if(parc > 2)
	    {
		unknown_list_allowed = !strncasecmp(parv[2], "ON", 2) ? YES : NO;
		sendto_realops_lev(SPAM_LEV, "%s said that unknown clients can%s issue /list command",
			parv[0], unknown_list_allowed ? "" : "not");
		sendto_one(sptr, ":%s NOTICE %s :unknown clients now can%s issue /list command",
			me.name, parv[0], unknown_list_allowed ? "" : "not");
	    }
	    else
	    {
		sendto_one(sptr, ":%s NOTICE %s :unknown clients are currently %sallowed to issue /list command",
			me.name, parv[0], unknown_list_allowed ? "" : "not ");
	    }
	...
	else if (!strncasecmp(command, "TUNIX", 5)) 
	{
	    extern int hide_tunix;
		...
		sendto_realops_lev(SPAM_LEV, "%s has changed tunix warez hiding to %s",
			parv[0], hide_tunix ? "ON" : "OFF");
		sendto_one(sptr, ":%s NOTICE %s :tunix warez hiding is now set to %s",
			   me.name, parv[0], hide_tunix ? "ON" : "OFF");
		return 0;
	    }
	    else 
	    {
		sendto_one(sptr, ":%s NOTICE %s :tunix warez hiding is currently %s",
			   me.name, parv[0], hide_tunix ? "enabled" : "disabled");
		return 0;
	    }
	}
	else if (!strncasecmp(command, "LANG", 5)) {

	    if (parc > 2) {

		if (!strcasecmp(parv[2], "IT")) {

		    if (CONF_SERVER_LANGUAGE == LANG_IT) {

			sendto_one(sptr, ":%s NOTICE %s :Server language is already set to Italian.",
				me.name, parv[0]);

			return 0;
		    }

		    CONF_SERVER_LANGUAGE = LANG_IT;

		    sendto_realops_lev(SPY_LEV, "%s has changed the server language to Italian", parv[0]);
		    sendto_one(sptr, ":%s NOTICE %s :Server language set to Italian.", me.name, parv[0]);
		}
		else if (!strcasecmp(parv[2], "EN")) {

		    if (CONF_SERVER_LANGUAGE == LANG_EN) {

			sendto_one(sptr, ":%s NOTICE %s :Server language is already set to English.",
				me.name, parv[0]);

			return 0;
		    }

		    CONF_SERVER_LANGUAGE = LANG_EN;

		    sendto_realops_lev(SPY_LEV, "%s has changed the server language to English", parv[0]);
		    sendto_one(sptr, ":%s NOTICE %s :Server language set to English.", me.name, parv[0]);
		}
		else {

		    sendto_one(sptr, ":%s NOTICE %s :Unknown language: %s",
		...
		sendto_one(sptr, ":%s NOTICE %s :Server language is currently set to %s",
			me.name, parv[0], (CONF_SERVER_LANGUAGE == LANG_IT) ? "Italian" : "English");
	...
	else if (!strncasecmp(command, "RESTRICT", 8)) 
	{
		...
		sendto_realops_lev(SPAM_LEV, "%s has changed unknown users restrictions to %s",
			parv[0], restriction_enabled? "ON" : "OFF");
		sendto_one(sptr, ":%s NOTICE %s :unknown users restrictions is now set to %s",
			   me.name, parv[0], restriction_enabled ? "ON" : "OFF");
		return 0;
	    }
	    else 
	    {
		sendto_one(sptr, ":%s NOTICE %s :unknown users restrictions is currently %s",
			   me.name, parv[0], restriction_enabled ? "enabled" : "disabled");
		return 0;
	    }
	}
#ifdef ANTI_SPAMBOT
	...
	else if(!strncasecmp(command, "CLOAK_KEY", 10) && IsAdmin(sptr))
	{
	   ...
	   /* several values available:
	    * ENABLE [on|off] to enable the code
	    * MIN [n] to set a min count, must be > 1
	    * MAX [n] to set a max count, must be > MIN
	    * MINTOMAXTIME [n] to set the time after which we mus force the MAX
	    * TIME [n] to set the throttle time
	    */
	   ...
	   if (parc > 3) {
		  ...
		  if (cnt > 1) {
	       ...
	       } else if (!strcasecmp(parv[2], "MAX")) {
		  int cnt;
		  changed = "MAX";
		  cnt = atoi(parv[3]);
		  if (cnt >= tlim_target_min) {
		     tlim_target_max = cnt;
		     to = parv[3];
		  } 
	       } else if (!strcasecmp(parv[2], "MINTOMAXTIME")) {
		  int cnt;
		  changed = "MINTOMAXTIME";
		  cnt = atoi(parv[3]);
		  if (cnt >= 30) {
		     tlim_target_mintomaxtime = cnt;
		     to = parv[3];
		  }
	       } else if (!strcasecmp(parv[2], "TIME")) {
		  int cnt;
		  ...
		  cnt = atoi(parv[3]);
		  if (cnt > 1) {
		     tlim_target_time = cnt;
		     to = parv[3];
		  }
	       }

	       if (to != NULL) {
		  sendto_ops("%s has changed targetlimit %s to %s", parv[0],
			changed, to);
		  sendto_one(sptr, ":%s NOTICE %s :set targetlimit %s to %s",
			me.name, parv[0], changed, to);
	       }
	   } else {
	      /* report various things */
	      sendto_one(sptr, ":%s NOTICE %s :TARGETLIMIT %s", me.name, parv[0],
		    tlim_enabled ? "enabled" : "disabled");
	      sendto_one(sptr, ":%s NOTICE %s :TARGETLIMIT MIN=%d target(s)", me.name,
		    parv[0], tlim_target_min);
	      sendto_one(sptr, ":%s NOTICE %s :TARGETLIMIT MAX=%d target(s)", me.name,
		    parv[0], tlim_target_max);
	      sendto_one(sptr, ":%s NOTICE %s :TARGETLIMIT MINTOMAXTIME=%d sec", me.name,
		    parv[0], tlim_target_mintomaxtime);
	      sendto_one(sptr, ":%s NOTICE %s :TARGETLIMIT TIME=%d sec", me.name,
	...
	sendto_one(sptr, ":%s NOTICE %s :Options: MAX <NUM>",
		   me.name, parv[0]);
#ifdef FLUD
	sendto_one(sptr, ":%s NOTICE %s :Options: FLUDNUM <NUM>, FLUDTIME <SECS>, "
		   "FLUDBLOCK <NUM>", me.name, parv[0]);
#endif
 
#ifdef ANTI_INVITE_FLOOD
	sendto_one(sptr, ":%s NOTICE %s :Options: INVITENUM <NUM>, INVITETIME <SECS>, "
	...
	sendto_one(sptr, ":%s NOTICE %s :Options: SPAMDETECT <ON|OFF>, TUNIX <ON|OFF>, LANG <IT|EN>, UNKNOWN_LIST <ON|OFF>",
		   me.name, parv[0]);
#ifdef DYNAMIC_CLOAKING
	sendto_one(sptr, ":%s NOTICE %s :Options: CLOAK_KEY <KEY>",
		   me.name, parv[0]);
#endif
#ifdef MSG_TARGET_LIMIT
...
#ifdef NO_CHANOPS_WHEN_SPLIT
	sendto_one(sptr, ":%s NOTICE %s :Options: SPLITDELAY <SECS>",
		   me.name, parv[0]);
#endif
...
int m_kline(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
    ...
    if (parc > 2) 
    {
	if (!temporary_kline_time && strchr(argv, IRCDCONF_DELIMITER)) 
	{
	    sendto_one(sptr,
		       ":%s NOTICE %s :Invalid character '%c' in comment",
		       me.name, parv[0], IRCDCONF_DELIMITER);
	    return 0;
	}

	if (!temporary_kline_time && strchr(argv, '#')) 
	{
	    sendto_one(sptr,
    ...
    (void) ircsprintf(buffer, "#%s!%s@%s K'd%c %s@%s%c%s\n",
		      sptr->name, sptr->user->username,
		      sptr->user->host,
		      IRCDCONF_DELIMITER, user, host,
    ...
    (void) ircsprintf(buffer, "K%c%s%c%s (%s)%c%s\n",
    		      IRCDCONF_DELIMITER,
		      host,
		      IRCDCONF_DELIMITER,
		      reason,
...
# endif
	...
	if ((buf[1] == IRCDCONF_DELIMITER) && ((buf[0] == 'k') || (buf[0] == 'K')))
	{
	    /* its a K: line */
		...
		*p = '\0';
	    ...
	    p = strchr(found_host, IRCDCONF_DELIMITER);
	    if (p == (char *) NULL) 
	    {
	    ...
	    *p = '\0';
	    ...
	    found_comment = p;
	    p = strchr(found_comment, IRCDCONF_DELIMITER);
	    if (p == (char *) NULL) 
	    {
		sendto_one(sptr, ":%s NOTICE %s :K-Line file corrupted",
			   me.name, parv[0]);
	...
	else if (buf[0] == '#') 
	{
	     ...
	     * ignore the line
	     */
	    p = strchr(buff, IRCDCONF_DELIMITER);
	    if (p == (char *) NULL) 
	    {
		if (!error_on_write)
		    error_on_write = flush_write(sptr, parv[0],
	    ...
	    *p = '\0';
	    ...
	    userathost = p;
	    p = strchr(userathost, IRCDCONF_DELIMITER);
...
int m_zline(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
    ...
    if (!MyClient(sptr) || !IsAnOper(sptr) || !OPCanZline(sptr))
    {
	sendto_one(sptr, err_str(ERR_NOPRIVILEGES), me.name, parv[0]);
	return 0;
    }
    ...
    {
	if (strchr(parv[2], IRCDCONF_DELIMITER)) 
	{
	    sendto_one(sptr,
		       ":%s NOTICE %s :Invalid character '%c' in comment",
		       me.name, parv[0], IRCDCONF_DELIMITER);
    ...
    (void) ircsprintf(buffer, "#%s!%s@%s D'd%c %s%c%s (%s)\n",
		      sptr->name, sptr->user->username,
		      sptr->user->host,
		      IRCDCONF_DELIMITER, host,
		      IRCDCONF_DELIMITER, reason,
    ...
    (void) ircsprintf(buffer, "D%c%s%c%s (%s)%c\n",
    		      IRCDCONF_DELIMITER,
		      host,
		      IRCDCONF_DELIMITER,
		      reason,
    ...
    (void) ircsprintf(buffer, "#%s!%s@%s Z'd%c %s%c%s (%s)\n",
		      sptr->name, sptr->user->username,
		      sptr->user->host, 
		      IRCDCONF_DELIMITER, host,
		      IRCDCONF_DELIMITER, reason,
    ...
    (void) ircsprintf(buffer, "Z%c%s%c%s (%s)%c\n", 
    		      IRCDCONF_DELIMITER, host,
		      IRCDCONF_DELIMITER, reason,
		      current_date,
		      IRCDCONF_DELIMITER);
...
int m_trace(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	if (IsAnOper(sptr) && (acptr->status == STAT_CLIENT
...
int m_die(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	for (i = 0; i <= highest_fd; i++) {
		...
		if (IsClient(acptr)) {
			...
			sendto_one(acptr, ":%s ERROR :Terminated by %s", me.name, get_client_name(sptr, TRUE));
	}
...
int m_sqline(aClient *cptr, aClient *sptr, int parc, char *parv[]) 
{
	...
	aconf->status = CONF_SQLINE |
	    (*mask == '#' ? CONF_QUARANTINED_CHAN : CONF_QUARANTINED_NICK);

	DupString(aconf->name, mask);
	DupString(aconf->passwd, (reason != NULL ? reason : "Reserved"));
	aconf->next = conf;
	conf = aconf;
	...
	for(i = 0; i <= highest_fd; i++)
	{
		...
		{
		    sendto_ops("SQ-Line active for %s", get_client_name(acptr, FALSE));
		    ircsprintf(fbuf, "SQ-Lined: %s", reason);
...
int m_sgline(aClient *cptr, aClient *sptr, int parc, char *parv[]) 
{
	...
	for(i = 0; i <= highest_fd; i++)
	{
	    ...
	    if(IsPerson(acptr))
	    {
		if(!match(mask, acptr->info))
		{
		    sendto_ops("SG-Line active for %s", get_client_name(acptr, FALSE));
		    ircsprintf(fbuf, "SG-Lined: %s", reason);
		    exit_client(acptr, acptr, &me, fbuf);
		    i--;
		}
	    }
 ...
 *  old protocol:
 * parv[1] = wildcard word to block
 * parv[2] = reason
 *  new protocol:
 * parv[1] = wildcard word to block
 * parv[2] = spam policy code (see struct.h/SPAM_*)
 * parv[3] = reason
 */
int m_spam(aClient *cptr, aClient *sptr, int parc, char *parv[]) 
{
    Spam *s = NULL;
    int type = SPAM_DEFAULT; /* Spam policy */
    char *reason;
    ...
    /* parc==3 only in old protocol -INT */
    if (parc>3)
    {              
	reason = parv[3];
	type = atoi(parv[2]);
	
	switch(type)  /* consistency checks */
	{
    ...
    for(s = spam_list; s != NULL; s = s->next)
    {
	if(!mycmp(s->msg, parv[1]))
	{
	    break;
	}
    }

    /* okay, it doesn't suck, build a new item for it */
    if(!s)
    {
	s = (Spam *)MyMalloc(sizeof(Spam));
	s->creationtime = NOW;
	s->count = 0;
	s->daycount = 0;
	s->weekcount = 0;
	s->monthcount = 0;
	s->type = type;
	DupString(s->msg, parv[1]);
	DupString(s->reason, reason ?: "Spam");
	...
	sendto_realops_lev(SPAM_LEV, 
			    "SPAM INFO: added new string: \"%s\" [%s] (type=%d)",
		s->msg, s->reason, s->type);
#endif
    }

    sendto_serv_butone(cptr, ":%s SPAM %s %d :%s", sptr->name,
	    s->msg, s->type, s->reason);
    return 0;
}
	
int m_unspam(aClient *cptr, aClient *sptr, int parc, char *parv[]) 
{
    char *mask;
    Spam *s, *prev = NULL;
	...
	if(!mycmp(mask, s->msg))
	{
...
#ifdef DEBUGMODE
	    sendto_realops_lev(SPAM_LEV, "SPAM INFO: Removed `%s' from spam strings", s->msg);
#endif
	    ...
	    sendto_serv_butone(cptr, ":%s UNSPAM :%s", sptr->name, mask);
